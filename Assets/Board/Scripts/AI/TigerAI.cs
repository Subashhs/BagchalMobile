...
public class TigerAI : BaseAI
{
    private GameObject tiger;
    private GameObject[] opponentGoats;
    private TigerMovementB4 tigerMovement;

    public TigerAI(GameObject tigerObject, GameObject[] goats, Dictionary<string, GameObject> boardTiles) : base(boardTiles)
    {
        tiger = tigerObject;
        opponentGoats = goats;
        if (tiger != null)
        {
            tigerMovement = tiger.GetComponent<TigerMovementB4>();
        }
    }
    ...
    public override bool MakeMove()
    {
        ...
        string currentTileName = GameManagerBoard4.Instance.GetTileName(tiger.transform.position);
        ...
        foreach (string move in possibleMoves)
        {
            ...
            if (!GameManagerBoard4.Instance.IsTileOccupied(targetTile))
            {
                ...
                {
                    GameObject jumpedGoat = GameManagerBoard4.Instance.GetAllGoats().FirstOrDefault(g => GameManagerBoard4.Instance.GetTileName(g.transform.position) == jumpedTileName);
                    if (jumpedGoat != null)
                    {
